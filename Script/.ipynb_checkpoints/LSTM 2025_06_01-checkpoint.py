...
def Train_Model(train_dataset,test_dataset,unique_times):
    ...
    for (hour,minute) in [(i.hour,i.minute) for i in unique_times]:
        ...
        # now_lstm_data.drop(['ficalQuarter', 'fiscalMonth', 'fiscalWeek', 'DOW','hour','minute'], axis=1, inplace=True) 
        # now_fill_na_predict.drop(['ficalQuarter', 'fiscalMonth', 'fiscalWeek', 'DOW','hour','minute'], axis=1, inplace=True)
        ...
        date_cols=['fiscalYear','ficalQuarter_sin','ficalQuarter_cos'             ,'fiscalMonth_sin','fiscalMonth_cos','fiscalWeek_sin','DOW_cos','DOW_sin','hour','minute','hour_sin',
       'hour_cos', 'minute_sin', 'minute_cos','datetime','date','offered']
        cal_cols=['fiscalYear','ficalQuarter_sin','ficalQuarter_cos'
              ,'fiscalMonth_sin','fiscalMonth_cos','fiscalWeek_sin','DOW_cos','DOW_sin','hour_sin',
